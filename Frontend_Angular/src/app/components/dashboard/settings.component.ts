      ...
      <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <select
            ...
            class="block rounded-xl border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800 focus:border-indigo-500 focus:outline-none min-w-[180px]"
          >
        ...
        <div class="border-t border-slate-100 dark:border-slate-800/60 pt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <select
            ...
            (change)="onCurrencyChange(settings.currency)"
            class="block rounded-xl border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800 focus:border-indigo-500 focus:outline-none min-w-[180px]"
          >
      ...
      <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
        ...
        <div class="border-t border-slate-100 dark:border-slate-800/60 pt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <select
            ...
            (change)="onSessionTimeoutChange(settings.sessionTimeout)"
            class="block rounded-xl border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800 focus:border-indigo-500 focus:outline-none min-w-[180px]"
          >
...
export class SettingsViewComponent implements OnInit {
  ...
  private loadSettings() {
    ...
    this.cs.setCurrency(this.settings.currency);
  ...
  protected onCurrencyChange(newCurrency: CurrencyCode) {
    this.cs.setCurrency(newCurrency);
    this.saveSettings();
  }

  protected toggleMaskBalance() {
    this.settings.maskBalance = !this.settings.maskBalance;
    this.cs.setMaskBalance(this.settings.maskBalance);
    this.saveSettings();
  }

  protected onSessionTimeoutChange(minutes: number) {
    this.sessionService.updateTimeout(minutes);
    this.saveSettings();
  }
  ...
  protected saveSettings(triggerToast = false) {
    ...
    localStorage.setItem('yono_currency', this.settings.currency);
    localStorage.setItem('yono_mask_balance', String(this.settings.maskBalance));
  ...
  protected resetDefaults() {
    ...
    this.sessionService.updateTimeout(15);
